...
public class ClipboardActionUtil {
    ...
    public void copy() {
        ...
        if (content != null) {
            ...
            if (config.message.logBasic) {
                Log.info(LogCategory.LOG, "Player copy the position content: " + content);
            }
            if (config.message.basic) {
                client.player.sendMessage(Text.of(new TranslatableText("text.location-clipboard.message.copy")
    ...
    public String copyPlayerLocation() {
        ...
        double pitch = player.getPitch(1);
        double yaw = player.getYaw(1);
